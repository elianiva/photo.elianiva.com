...
- Open `https://photo.localhost/admin` (`portless get photo` + `/admin`) — the filter bar shows every Tag as a chip with a count when unfiltered, or as a single active chip when filtered.
