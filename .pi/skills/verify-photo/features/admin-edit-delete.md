...
- **Proof.** Capture Sheet open and post-save grid states. Run `npx agent-browser snapshot > .pi/skills/verify-photo/artifacts/admin-edit-delete/sheet.aria.txt` and `npx agent-browser screenshot .pi/skills/verify-photo/artifacts/admin-edit-delete/sheet.png` for the open sheet, plus a second snapshot/screenshot after save showing the updated tile in the grid.
