...
export const foldUploadDialog = Update.foldChild({
  ...
  write: (model: Model, nextDialog: typeof model.uploadDialog) =>
    modifyFields(model, { uploadDialog: () => nextDialog }),
  ...
  foldOutMessage: (out): Update.Step<Model, Msg> =>
    out._tag === 'Closed'
      ? (writtenModel) => ({
          ...
          model: writtenModel.uploading ? writtenModel : releaseFinishedItems(writtenModel),
        })
      : (writtenModel) => ({ model: writtenModel }),
})
...
export const foldConfirm = Update.foldChild({
  ...
  write: (model: Model, nextDialog: typeof model.confirmDialog) =>
    modifyFields(model, { confirmDialog: () => nextDialog }),
  toParentMessage: (message: typeof Dialog.Message.Type) => Message.GotConfirmMessage({ message }),
  foldOutMessage: (out): Update.Step<Model, Msg> =>
    out._tag === 'Closed'
      ? (writtenModel) => ({
          model: modifyFields(writtenModel, { pendingConfirm: () => undefined }),
        })
      : (writtenModel) => ({ model: writtenModel }),
})
...
export const foldFileDrop = Update.foldChild({
  ...
  write: (model: Model, nextDrop: typeof model.fileDrop) =>
    modifyFields(model, { fileDrop: () => nextDrop }),
  ...
  foldOutMessage: (out: typeof FileDrop.OutMessage.Type): Update.Step<Model, Msg> => {
    if (out._tag !== 'ReceivedFiles') {
      ...
      return (droppedModel) => ({ model: droppedModel })
    }
    ...
    return (droppedModel) => {
      ...
      if (duplicateCount > 0) {
        const toastShown = showToast(
          nextModel,
          `${String(duplicateCount)} duplicate${duplicateCount === 1 ? '' : 's'} skipped`,
          'Error',
          'Already in the upload queue.',
        )
        nextModel = toastShown.model
        mutableCommands = [...mutableCommands, ...(toastShown.commands ?? [])]
      }
      if (oversized.length > 0) {
        const oversizedToast = showToast(
          nextModel,
          `${String(oversized.length)} file(s) too large`,
          'Error',
          `Max ${String(UPLOAD_LIMITS.maxFileSize / (1024 * 1024))} MB per file.`,
        )
        nextModel = oversizedToast.model
        mutableCommands = [...mutableCommands, ...(oversizedToast.commands ?? [])]
      }
      if (overflowCount > 0) {
        const overflowToast = showToast(
          nextModel,
          `Only ${String(UPLOAD_LIMITS.maxFiles)} files allowed`,
          'Error',
          `${String(overflowCount)} file(s) skipped — remove some to add more.`,
        )
        nextModel = overflowToast.model
        mutableCommands = [...mutableCommands, ...(overflowToast.commands ?? [])]
      }
      return mutableCommands.length > 0
        ? { model: nextModel, commands: mutableCommands }
        : { model: nextModel }
    }
  },
})
...
export const foldToast = Update.foldChild({
  ...
  write: (model: Model, nextToast: typeof model.toast) =>
    modifyFields(model, { toast: () => nextToast }),
