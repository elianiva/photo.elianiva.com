...
const masthead = (h: HtmlBuilder<Message>): Child =>
  h.header([h.Class(`${GUTTER} pt-10 lg:pt-14 flex items-baseline justify-between`)], [
    h.a(
      [
        h.Href('/'),
        h.Class(
...
const hero = (model: Model, h: HtmlBuilder<Message>): Child => {
  ...
  return h.section([h.Class(`${GUTTER} pt-16 pb-20 lg:pt-24 lg:pb-32`)], [
    h.h1(
      [
        h.Class(
...
const footer = (h: HtmlBuilder<Message>): Child =>
  h.footer(
    ...
    [
      h.span([h.Class('text-[10px] uppercase tracking-[0.3em] text-neutral-400')], [
        `© ${String(currentYear())} Elianiva`,
      ]),
      h.span([h.Class('font-serif italic font-light text-sm text-neutral-300')], [
        'All photographs.',
      ]),
    ],
  )
...
export const view = (model: Model, h: HtmlBuilder<Message>): Document => ({
  ...
  body: h.div([h.Class('min-h-screen bg-white text-neutral-900')], [
    masthead(h),
    hero(model, h),
